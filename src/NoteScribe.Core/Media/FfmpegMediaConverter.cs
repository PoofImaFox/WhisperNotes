        ...
        using (document)
        {
            ...
            foreach (var stream in streams.EnumerateArray())
            {
                ...
                if (stream.TryGetProperty("sample_rate", out var rateElement))
                {
                    ...
                    else if (rateElement.ValueKind == JsonValueKind.Number)
                    {
                        rateElement.TryGetInt32(out sampleRate);
                    }
                }
                ...
                results.Add(new MediaAudioStream(index, codec, channels, sampleRate, language, title));
            }
