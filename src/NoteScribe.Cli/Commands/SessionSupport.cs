...
internal static class SessionSupport
{
    ...
    public static NoteEntry ToEntry(NoteSession session, TranscriptSegment segment) => new(
        ...
        segment.Text,
        // Speaker attribution is a later phase; an invented name would poison a billable record.
        Speaker: null,
        segment.Confidence);

    public static string NotesPath(NoteScribeServices services, string sessionId) =>
        Path.Combine(services.Notes.GetSessionDirectory(sessionId), FileSystemNoteRepository.NotesFileName);

    public static string AudioPath(NoteScribeServices services, string sessionId) =>
        Path.Combine(
    ...
    public static CliException UnknownChannel(string requested, IReadOnlyList<ChannelEntry> entries)
    {
        ...
        return new CliException(
            ...
            $"No audio endpoint matches '{requested}'. Valid ids (see 'notescribe devices'):"
