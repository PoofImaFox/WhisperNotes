...
internal static class TranscribeCommand
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var wavPath = keepAudio
            ...
            : Path.Combine(Path.GetTempPath(), $"notescribe-{session.Id}.wav");
        ...
        try
        {
            ...
            entries = await DecodeAsync(services, console, session, wavPath, cancellationToken).ConfigureAwait(false);
        }
        finally
        {
            if (!keepAudio)
        ...
        CancellationToken cancellationToken)
    {
        ...
        ProgressIndicator indicator = new(console, "extracting", LabelWidth);
        try
        {
        ...
        CancellationToken cancellationToken)
    {
        ...
        ProgressIndicator indicator = new(console, "transcribing", LabelWidth);
    ...
    private static TimeSpan? TryGetDuration(NoteScribeServices services, string wavPath)
    {
        try
        {
    ...
    private static void RequireFfmpeg(NoteScribeServices services, ConsoleOutput console)
    {
