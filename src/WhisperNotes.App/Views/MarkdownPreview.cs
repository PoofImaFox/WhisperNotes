...
public sealed class MarkdownPreview : Decorator
{
    ...
    private Control BuildTable(MdTable table, Palette theme)
    {
        ...
        for (var r = 0; r < rows.Count; r++)
        {
            ...
            for (var c = 0; c < columns; c++)
            {
                ...
                var edges = new Thickness(0, 0, c == columns - 1 ? 0 : 1, r == rows.Count - 1 ? 0 : 1);
                var border = BuildCell(rows[r], c, edges, theme);
    ...
    /// <summary>
    /// One table cell. A row may be short of the widest row, so a missing cell renders as an empty
    /// one rather than shifting the columns that follow it.
    /// </summary>
    private Border BuildCell(MdTableRow row, int column, Thickness edges, Palette theme)
    {
        var content = new StackPanel { Spacing = 0 };

        if (column < row.Count && row[column] is MdTableCell cell)
        {
        ...
        {
            last.Margin = new Thickness(0);
            last.FontWeight = row.IsHeader ? FontWeight.SemiBold : FontWeight.Normal;
            last.Foreground = row.IsHeader ? theme.TextSecondary : theme.TextPrimary;
        }
        ...
        return new Border
        {
            ...
            Background = row.IsHeader ? theme.SurfaceRaised : Brushes.Transparent,
    ...
    private void AppendInlines(InlineCollection target, ContainerInline container, Palette theme, InlineStyle style)
    {
        ...
        {
            AppendInline(target, inline, theme, style);
        }
    }

    /// <summary>
    /// One node of the inline tree. Deliberately a flat, exhaustive dispatch on the Markdig type:
    /// the arms are ordered most-specific first, and the trailing <c>ContainerInline</c> /
    /// <c>LeafInline</c> arms are the catch-alls that keep an unrecognised node visible as text.
    /// </summary>
    private void AppendInline(InlineCollection target, MdInline inline, Palette theme, InlineStyle style)
    {
        switch (inline)
        {
            ...
            case LineBreakInline lineBreak:
                EmitBreak(target, lineBreak.IsHard, theme, style);
                break;
    ...
    /// <summary>
    /// A hard break — two trailing spaces or a backslash — is a real newline. A soft one is only
    /// where the author happened to wrap the source, so it renders as the space between two words.
    /// </summary>
    private void EmitBreak(InlineCollection target, bool isHard, Palette theme, InlineStyle style)
    {
        if (isHard)
        {
