...
public sealed partial class NoteEditorViewModel : ObservableObject, IAsyncDisposable
{
    ...
    /// <summary>
    /// Applies a deliberate whole-document user edit and records it as one named revision. Pending
    /// typing is flushed first so it remains independently recoverable in History.
    /// </summary>
    public async Task<bool> ApplyUserEditAsync(string updatedContent, string revisionLabel)
    {
        ...
        if (string.Equals(updatedContent, _content, StringComparison.Ordinal))
        {
            return true;
        }
        ...
        TextReplaced?.Invoke(this, updatedContent);
        PreviewMarkdown = updatedContent;
        CancelAutosave();

        // If the write fails, SaveCoreAsync deliberately leaves Content dirty and visible so the
        // correction is never lost merely because persistence was temporarily unavailable.
        return await SaveCoreAsync(revisionLabel.Trim(), NoteRevisionOrigin.User).ConfigureAwait(true);
    }
    ...
    /// <summary>Everything one run needs, settled before a single token is sent.</summary>
    private readonly record struct ActionRunContext(
        AiAction Action,
    ...
    {
        if (!TryPrepareAction(item, out var context))
        {
            return;
        }
        ...
        await StreamIntoPreviewAsync(item, context, request).ConfigureAwait(true);
    }

    /// <summary>
    /// Settles what the run will operate on, or explains why it cannot happen and returns false.
    /// Every rejection here is an ordinary user situation — no selection, an empty note, no provider
    /// — so each one says what to do next rather than failing silently.
    /// </summary>
    private bool TryPrepareAction(AiActionViewModel item, out ActionRunContext context)
    {
        context = default;
        ...
        context = new ActionRunContext(action, assistant, document, target, instruction, useSelection);
        return true;
    }

    private AiRequest BuildRequest(in ActionRunContext context)
    {
        ...
        var prompt = AiActionCatalog.Render(action.UserPromptTemplate, new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ...
            ["selection"] = context.UseSelection ? context.Target : string.Empty,
            ["title"] = context.Document.Title,
            ["project"] = context.Document.Project ?? string.Empty,
            ["instruction"] = context.Instruction,
        });

        return new AiRequest(action.SystemPrompt, [AiMessage.User(prompt)], _maxOutputTokens());
    }

    /// <summary>
    /// Empties the preview pane and pins where this run's answer would land, so Apply and Insert
    /// still target the original selection after the caret has moved on.
    /// </summary>
    private void ArmPreview(in ActionRunContext context)
    {
        var action = context.Action;
        ...
        _resultUsedSelection = context.UseSelection;

        _stream.Clear();
        PreviewText = string.Empty;
        PreviewNote = null;
        PreviewHeader = $"{action.Icon}  {action.Name} · {(context.UseSelection ? "selection" : "whole note")}";
    }

    private async Task StreamIntoPreviewAsync(AiActionViewModel item, ActionRunContext context, AiRequest request)
    {
        var action = context.Action;

        // Scoped to the run: whoever started the source disposes it, so a window closing mid-run
        // cannot leave one behind and a second run never adopts the first one's registrations.
        ...
        try
        {
            await foreach (var fragment in context.Assistant.StreamAsync(request, cts.Token).ConfigureAwait(true))
            {
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        if (_previewDebounce is { } preview)
        {
        ...
        // A run still streaming disposes its own source as it unwinds, but that lands after we have
        // returned; drop the handle here so no source outlives the view model. Dispose is idempotent,
        // so the two paths meeting is harmless.
        _run?.Dispose();
        _run = null;
