...
public sealed partial class NotesWorkspaceViewModel : ObservableObject, IAsyncDisposable
{
    ...
    public ObservableCollection<string> TranscriptSpeakers { get; } = [];
    ...
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(CanExportNote))]
    [NotifyPropertyChangedFor(nameof(CanExportLibrary))]
    ...
    [ObservableProperty]
    [NotifyCanExecuteChangedFor(nameof(BeginTranscriptSpeakerRenameCommand))]
    public partial string? SelectedTranscriptSpeaker { get; set; }

    [ObservableProperty] public partial bool IsRenamingTranscriptSpeaker { get; set; }

    [ObservableProperty] public partial string TranscriptSpeakerRenameText { get; set; } = string.Empty;

    public bool HasDocuments => Documents.Count > 0;

    public bool HasSelection => SelectedDocument is not null;

    public bool HasTranscriptSpeakers => TranscriptSpeakers.Count > 0;

    public bool CanExportNote => Editor.HasDocument && !IsExporting;

    public bool CanExportLibrary => !IsExporting;
    ...
    private bool HasTranscriptSpeakerSelection => SelectedTranscriptSpeaker is { Length: > 0 };

    partial void OnSelectedTranscriptSpeakerChanged(string? value) =>
        IsRenamingTranscriptSpeaker = false;

    [RelayCommand(CanExecute = nameof(HasTranscriptSpeakerSelection))]
    private void BeginTranscriptSpeakerRename()
    {
        if (SelectedTranscriptSpeaker is not { Length: > 0 } speaker)
        {
            return;
        }

        TranscriptSpeakerRenameText = speaker;
        IsRenamingTranscriptSpeaker = true;
    }

    [RelayCommand]
    private void CancelTranscriptSpeakerRename() => IsRenamingTranscriptSpeaker = false;

    [RelayCommand]
    private async Task CommitTranscriptSpeakerRenameAsync()
    {
        if (SelectedTranscriptSpeaker is not { Length: > 0 } previous)
        {
            IsRenamingTranscriptSpeaker = false;
            return;
        }

        // A single-line label cannot safely carry pasted newlines or control whitespace into the
        // Markdown delimiter. Collapse all whitespace runs while preserving ordinary punctuation.
        string name = string.Join(
            ' ',
            TranscriptSpeakerRenameText.Split((char[]?)null, StringSplitOptions.RemoveEmptyEntries));
        ...
        string updated = TranscriptSpeakerMarkup.Rename(
            Editor.Content,
            previous,
            name,
        ...
        {
            IsRenamingTranscriptSpeaker = false;
            RefreshTranscriptSpeakers();
            return;
        }

        bool saved = await Editor
            .ApplyUserEditAsync(updated, $"Rename speaker {previous} to {name}")
            .ConfigureAwait(true);

        IsRenamingTranscriptSpeaker = false;
        RefreshTranscriptSpeakers(preferred: name);
        ...
        if (saved)
        {
            _notify(
                ...
                "Speaker renamed",
                replacements == 1
                    ? $"Changed the {previous} label to {name}."
                    : $"Changed {replacements:N0} {previous} labels to {name}.");
        }
    }

    private void RefreshTranscriptSpeakers(string? preferred = null)
    {
        string? keep = preferred ?? SelectedTranscriptSpeaker;
        IReadOnlyList<string> found = Editor.Document?.SourceSessionId is { Length: > 0 }
            ? TranscriptSpeakerMarkup.FindSpeakers(Editor.Content)
            : [];

        if (!found.SequenceEqual(TranscriptSpeakers, StringComparer.Ordinal))
        {
            TranscriptSpeakers.Clear();
            foreach (string speaker in found)
            {
                TranscriptSpeakers.Add(speaker);
            }
        }

        SelectedTranscriptSpeaker = keep is { Length: > 0 }
                                    && TranscriptSpeakers.Contains(keep, StringComparer.Ordinal)
            ? keep
            : TranscriptSpeakers.FirstOrDefault();

        if (TranscriptSpeakers.Count == 0)
        {
            IsRenamingTranscriptSpeaker = false;
        }

        OnPropertyChanged(nameof(HasTranscriptSpeakers));
    }
    ...
    /// <summary>Prevents a double-click from opening overlapping platform save pickers.</summary>
    ...
    /// <summary>Called by the view after the platform picker closes, succeeds or fails.</summary>
    public void FinishExport() => IsExporting = false;

    /// <summary>
    /// Flushes the editor and builds a portable artifact for the open note. The view owns the
    /// platform save picker; returning null means a failure was already surfaced to the shell.
    /// </summary>
    public async Task<NoteExportArtifact?> PrepareNoteExportAsync(NoteExportFormat format)
    {
        try
        {
            if (Editor.Document is null)
            {
                _notify(NotificationSeverity.Warning, "No note to export", "Open a note and try again.");
                return null;
            }

            if (!await Editor.TryFlushAsync().ConfigureAwait(true))
            {
                return null;
            }

            // TryFlushAsync replaces Editor.Document with the saved head, so the export includes
            // the last keystroke even when the autosave debounce has not elapsed.
            return _services.Exports.Export(Editor.Document!, format);
        }
        catch (Exception ex)
        {
            _notify(NotificationSeverity.Error, "Could not export that note", ex.Message);
            return null;
        }
    }

    /// <summary>
    /// Flushes the open note, then deliberately asks the store for an unfiltered list. Search
    /// results are a UI projection and must never narrow a whole-library Obsidian export.
    /// </summary>
    public async Task<NoteExportArtifact?> PrepareObsidianExportAsync()
    {
        try
        {
            if (!await Editor.TryFlushAsync().ConfigureAwait(true))
            ...
            var allDocuments = await _store.ListAsync(null, CancellationToken.None).ConfigureAwait(true);
            if (allDocuments.Count == 0)
            {
                _notify(NotificationSeverity.Warning, "No notes to export", "The note library is empty.");
                return null;
            }

            return _services.Exports.ExportObsidian(allDocuments);
        }
        catch (Exception ex)
        {
            _notify(NotificationSeverity.Error, "Could not export the note library", ex.Message);
            return null;
        }
    }

    /// <summary>Reports a completed platform write through the shell's notification stack.</summary>
    public void ReportExportSaved(string fileName, bool isLibrary)
    {
        _notify(
            NotificationSeverity.Info,
            isLibrary ? "Obsidian export saved" : "Note export saved",
            $"Saved “{fileName}”.");
    }

    /// <summary>Reports picker or stream failures; picker cancellation intentionally never calls this.</summary>
    public void ReportExportSaveFailure(Exception exception, bool isLibrary)
    {
        ArgumentNullException.ThrowIfNull(exception);
        _notify(
            NotificationSeverity.Error,
            isLibrary ? "Could not save the Obsidian export" : "Could not save the note export",
            exception.Message);
    }
    ...
    private void OnEditorPropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        else if (e.PropertyName is nameof(NoteEditorViewModel.HasDocument))
        {
            OnPropertyChanged(nameof(CanExportNote));
            RefreshTranscriptSpeakers();
        }
        else if (e.PropertyName is nameof(NoteEditorViewModel.Content))
        {
            RefreshTranscriptSpeakers();
        }
    }
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        // CancelAsync, not Cancel: Cancel runs registered callbacks inline on the caller, which here
        // is the UI thread, and a slow one would stall the window as it closes.
        if (_searchDebounce is { } search)
        {
            await search.CancelAsync().ConfigureAwait(true);
        }

        _searchDebounce?.Dispose();
        _searchDebounce = null;

        if (_markerScan is { } scan)
        {
